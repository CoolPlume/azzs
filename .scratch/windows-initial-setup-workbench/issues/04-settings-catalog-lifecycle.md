...
- [x] 更新前分别预览新增、变更和下架项目，用户确认后才加载。
- [x] 整体错误或未知执行语义拒绝目录并保留上一份可用版本；未知展示或说明内容可忽略。
...
- [x] 加载、升降级目录不会自动修改当前系统优化状态。
- [ ] 目录变化不会破坏已应用设置的恢复记录或撤销能力。
- [x] 软件与驱动目录或软件优化目录的变化不联动改变系统设置目录，系统设置目录变化也不联动改变另外两类目录。
...
- 2026-08-12：Resolution completed。PR #6 已 Squash 为 `7de79a526a48c1bd0a36fbaf74b13432b65251c8`；integration run `31510915221` 的 x64/ARM64 均成功。`settings-catalog.contract` 自动覆盖更新预览/确认、整包拒绝、回退与目录隔离，并验证更新不自动改变当前系统优化状态。
- 未验证边界：未进行真实 Windows 设置写入与撤销、真实持久化/多进程恢复、WinUI 普通/高级视图与 DPI 或设备实机验证；未获完整自动证据的验收项保持未勾选。
