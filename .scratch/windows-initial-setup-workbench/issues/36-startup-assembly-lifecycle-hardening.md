...
- 2026-08-20：启动修复 feature `3a8858b1003f66b95f47b19264923b0dc1ec5b0c` 已普通合入 integration `8c1bc78`；该提交树与最终 feature 树一致。x64 Release OFF EXE 运行 25.053 秒，窗口“Windows 初装工作台”保持有效、`Responding=true` 且进程未退出；CDB 命中 `wWinMain`、`App::OnLaunched`、`assemble_startup`、`MainWindow` 构造及 `show_initial_page`，未命中 `0x802B000A`、`C000027B` 或 `C0000409`。OFF/ON manifest 均绑定 `sourceCommit=3a8858b`、`dirty=false`；portable/installer 对诊断构建的拒绝门禁通过。该补充只刷新本事项的 x64 启动证据，不把 ARM64、ON EXE、MSI/WiX、完整便携候选、安装生命周期或 CI 写成已验证。
- 2026-08-20：针对精确 integration 源码提交 `4a3c288e39ca07771b8d06acda9a2dd4bd99d83f` 的 D 盘独立 worktree 完成 x64 Release 构建与运行时采样。manifest 为 `source.dirty=false`；`Azzs.WinUI.exe` 为 PE Machine `0x8664`（AMD64/x64），SHA-256 为 `F170F97813E8EDFAA7D6B68886A422CFBB09844A01F0013CB7C3E9C30109B77B`。两次受控启动均未复现“启动即退出”：首轮约 12.5 秒的 12 个样本均 `HasExited=false`，窗口句柄 `0x1C1356`、标题“Windows 初装工作台”、`Responding=true`；第二次约 3 秒快照同样存活并有响应窗口。进程由采样脚本主动终止，记录的 `exitCode=-1` 不是应用自然退出码。当前环境未找到可执行 `cdb.exe`、传统 `windbg.exe` 或 `ntsd.exe`，`WinDbgX.exe` 仅为 C 盘 0 字节入口，因此没有入口断点或异常码调试器证据；未复现自然退出也不等于已通过调试器排除 `0xC000027B`、`0x802B000A` 或 `0xC0000409`。完整证据见 `D:\azzs-codex\diagnostics\startup-runtime-repro-4a3c288\startup-runtime-evidence.md` 及同目录 JSON；本轮未修改源码，C 盘仅只读，ARM64、MSI/WiX、安装/升级/卸载生命周期和发布候选仍未验证，事项 `Resolution: completed` 与 `Blocked by: 01, 31` 保持不变。
