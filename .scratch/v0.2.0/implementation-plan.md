...
- 显示器摘要为型号加括号内分辨率、Hz、内建/外接和英寸四槽，固定顺序为“分辨率；Hz；内建/外接；英寸”；英寸优先由可验证的 EDID 物理尺寸换算，缺失、无效或冲突时才由活动 WMI 物理尺寸回退，且只接受完整 PNP 实例精确匹配，不能按同型号共享；无法可靠取得时对应槽显示“未识别”字段而不猜测或省略，原始 EDID/实例标识继续省略；无记录的多实例类别隐藏。
