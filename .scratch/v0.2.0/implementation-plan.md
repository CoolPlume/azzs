...
| 3 | 事项 03：硬件表字段收敛、选择复制、GPU/输入设备和显示器摘要 | 0 | Terra Max 或 XHigh；Windows 观测变化与 UI 分离 |
...
- 以结构化硬件事实为唯一来源，统一“项目 | 信息”两列表，加入机型和 Windows 版本表头，显示 CPU、主板、内存、显卡、显示器、固态硬盘、机械硬盘、有线/无线网卡、声卡、NPU、键盘、鼠标和触控板。
- CPU 摘要固定显示中文的性能核、能效核和低功耗能效核三段；仅当 Windows 可靠确认两类或三类拓扑且总数一致时显示 `N性能核+N能效核+N低功耗能效核`，缺失类别明确为 `0`，不得因零值省略；拓扑不可获取、单一 `EfficiencyClass` 或计数不一致时三段均显示“未识别”，不猜测或以 `0` 代替未知。
- 删除普通页面的“未分类物理磁盘”、接口/PCIe、NAND 颗粒、资料识别和研究说明；不得破坏 ADR-0049 的物理性 fail-closed 过滤。
- 显示器摘要为型号加括号内分辨率、刷新率（Hz 数值）、内建/外接和英寸四槽，固定顺序为“分辨率；Hz；内建/外接；英寸”；英寸优先由可验证的 EDID 物理尺寸换算，缺失、无效或冲突时才由活动 WMI 物理尺寸回退，且只接受完整 PNP 实例精确匹配，不能按同型号共享；无法可靠取得时对应槽显示“未识别”字段而不猜测或省略，原始 EDID/实例标识继续省略；无记录的多实例类别隐藏。
- 核显的模型、架构及 CU/EU/Xe 仅由可靠事实投影；独显不投影计算单元或共享显存，GPU 板卡厂商也不能由型号、泛化 WMI 文本或资料库推断。设备型号移除精确的 `(R)`/`(TM)` 标记，容量从可靠字节事实规范化；只有可靠身份才显示“英睿达 Crucial”。
- 新增键盘、鼠标和触控板的结构化事实与表格投影。键盘必须有可靠的内建/外接状态，键数只在可靠观测时显示；内建类型优先使用设备、OEM 或驱动给出的具体类型，无可靠类型只显示“内建键盘”，不从整机型号、WMI 名称或厂商品牌推断。
- 让信息值支持鼠标/键盘选择复制，不增加“复制本节”按钮或命令；剪贴板只在 UI 适配器实现，不进入核心状态。
...
- 核心/投影合同覆盖字段删除、CPU 中文零值、核显/独显边界、GPU 板卡厂商、型号/容量/品牌规范化、键盘/鼠标/触控板、未识别/空多实例、显示器多实例和未知 EDID。
- Windows x64 实机验证多显示器、硬件值选择复制且无专用整节复制控件、无未分类磁盘/NAND/资料识别；记录剪贴板文本和每项设备事实来源。GPU 与内建键盘字段未能可靠取得时必须记录为未验证或未识别，而非补推。
- grep 页面资源和投影代码，确认不再出现禁用标签或推断字符串；不得以删除 UI 文本掩盖核心事实契约错误。

### 防回归

不要从显示字符串反解析或重新猜测物理类别；不要从整机型号、WMI 名称或厂商品牌推断内建键盘类型、GPU 架构、计算单元、板卡厂商或键数；不要显示序列号、原始 EDID、唯一标识或把诊断研究说明混入普通页面。
