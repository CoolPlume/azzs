...
- 代理拥有的 worktree、日志、缓存、提示词、临时输出和构建输出只能位于 `D:\azzs-codex` 下。新建执行 worktree 使用 `D:\azzs-codex\worktrees\<feature-slug>`；`D:\azzs-codex` 是代理产物容器，不是第二个独立仓库。
- 代理不得在 `C:\` 或 `D:\` 根目录创建目录或文件。所有新建路径必须位于上述已指定的项目源根目录或代理产物容器内。
- 代理不得创建或使用盘符映射，包括 `subst`、`net use` 映射、挂载点盘符或任何等效别名；始终使用完整的 `D:\azzs-codex\...` 路径。
- 不把 worktree 放在网络共享、临时盘、同步盘或会自动改变路径的目录中；不得使用 junction、符号链接或其他别名伪造仓库根路径。
- 既有用户拥有的路径不得因代理工作而移动或删除。
