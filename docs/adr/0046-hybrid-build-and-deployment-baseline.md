...
首个应用骨架采用一条混合但唯一的 Windows 构建路径：CMake 与 CTest 拥有可移植领域、应用核心、无界面测试和非 XAML Windows 适配器，MSBuild 拥有 C++/WinRT、XAML 宿主与最终可执行文件，`eng/build.ps1` 是连接两者的唯一非交互入口。Windows 候选固定为 Visual Studio 2026 Stable 18.8.2、其稳定内置 CMake 4.3.1-msvc1、稳定 MSVC 14.50/v145、Windows SDK release 10.0.28000.2526（MSBuild target 10.0.28000.0）、Microsoft.WindowsAppSDK 2.3.1 与 Microsoft.Windows.CppWinRT 3.0.260715.1；MSVC 的 C++26 目标映射为 `/std:c++latest`，在正式 `/std:c++26` 可用前只采用已稳定实现且不进入持久格式或稳定 ABI 的语言能力。
