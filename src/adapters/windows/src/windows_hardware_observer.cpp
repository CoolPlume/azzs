...
void rebuild_summary(
    ...
    std::optional<application::HardwareInputDeviceType> input_type =
        std::nullopt) {
  target.clear();
  std::vector<std::string> emitted_input_names;
  for (auto const& device : devices) {
    ...
    auto name = grouped_name(device);
    // Multiple WMI projections can describe the same input category. Keep
    // every structured device record, but collapse identical compact labels.
    if (input_type.has_value() &&
        std::ranges::find(emitted_input_names, name) !=
            emitted_input_names.end()) {
      continue;
    }
    if (!target.empty()) {
      target.push_back('\n');
    ...
    target.append(name);
    if (input_type.has_value()) {
      emitted_input_names.push_back(std::move(name));
    }
  }
}
